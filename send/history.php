    ...
    while($row = $result->fetch_array(MYSQLI_ASSOC)) {
        ...
        if($uid !== $row["uid"]) {
            if($uid !== "") {
                ...
                echo '<td title="Копировать" class="id">' . $uid . '</td><td>' . $username . '</td><td class="text">' . ($list != 'Blank' ? "<span>$list ($list_id)</span>" : '') . "$text $filename" . ($answer != '' ? "<span class=\"answer\">$answer</span>" : '') . "</td><td><a href=\"http://omssis-sms.mts-nn.ru/sms2/send/copy.php?argus=$argus\" target=\"_blank\">$argus</a>$recovery</td><td>$name</td><td>$sent</td><td>$done</td><td>$status</td>";
                echo "</tr>\n";
            }
            $answer = '';
            $i++;
